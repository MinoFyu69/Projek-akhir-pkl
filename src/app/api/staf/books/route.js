...
export async function POST(req) {
	...
	const { title, author, genreId, stock = 0, tagIds = [], requestedBy = 'staf' } = body || {};
	if (!title) return NextResponse.json({ message: 'title required' }, { status: 400 });

	let createdId;
	withTransaction(() => {
		const insert = db.prepare(`INSERT INTO books (title, author, genre_id, stock, is_approved) VALUES (?, ?, ?, ?, 0)`);
		...
		db.prepare(`INSERT INTO approvals (book_id, requested_by, status) VALUES (?, ?, 'pending')`).run(createdId, requestedBy || role);
	});

	const row = db.prepare(`SELECT * FROM books WHERE id = ?`).get(createdId);
	return NextResponse.json({ ...mapBook(row), approvalStatus: 'pending' }, { status: 201 });
}
...
export async function PUT(req) {
	...
	const db = getDb();
	const body = await req.json();
	const { id, title, author, genreId, stock, tagIds } = body || {};
	...
	withTransaction(() => {
		...
		db.prepare(`UPDATE books SET title = @title, author = @author, genre_id = @genre_id, stock = @stock WHERE id = @id`).run({ ...next, id });
